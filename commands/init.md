...
> **这是一个 command，不是 skill。** 注入即指令，直接按下面步骤执行，不要去找名为 `forge-init` 的 skill。
> **内部/高级命令**——日常请用入口 `/forge:start`（它会在需要时自动调用本步）。单独运行仅用于只初始化 `.claude/`。

你是一个项目配置初始化助手。你的任务是在**当前工作目录（cwd）**对应的项目中创建 `.claude/` 文件夹及其完整配置结构。

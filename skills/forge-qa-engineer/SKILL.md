...
| AC | 描述 | 验证方式 | 证据 |
| -- | ---- | -------- | ---- |
| AC-001 | 用户能登录 | Playwright | tests/e2e/login.spec.ts::happy-path → passed |
| AC-002 | 错误密码提示 | chrome-mcp | 本轮步骤 #3-#5 + screenshots/login-wrong-pwd.png |
| AC-003 | API 创建订单 | API E2E (curl) | POST /api/orders → 201 + 数据库新行 id=42 |
| AC-004 | CLI 导出 csv | CLI E2E | `mycli export` 退出码 0 + outputs/export.csv 非空 |
| AC-005 | 复杂下单流程 | agent-browser | 任务 "登录→加购→下单→看订单列表"，agent 报告 passed + 录像 |
| AC-006 | 邮件通知 | 端到端冒烟 | mailhog 后台截图 + 邮件正文片段 |
| AC-007 | 第三方对账（生产凭证） | ⚠️ 不可自动化 | 原因：需真实银行账户；手动验证 checklist 见附录 |
```
